...
const MENU_DATA: Record<string, any> = {
  coffee_0: { name: "衣索比亞 耶加雪菲", price: 180, desc: "花香調、柑橘風味、清爽回甘。", nutrition: { kcal: 5, carb: 1, fat: 0, protein: 0.5 } },
  coffee_1: { name: "哥倫比亞 聖奧古斯丁", price: 190, desc: "堅果巧克力調、醇厚度高。", nutrition: { kcal: 8, carb: 1.5, fat: 0.2, protein: 0.6 } },
  coffee_2: { name: "貓咪拉花拿鐵", price: 160, desc: "綿密奶泡繪製超萌店貓圖案。", nutrition: { kcal: 150, carb: 12, fat: 8, protein: 6 } },
  coffee_3: { name: "貓掌拿鐵", price: 170, desc: "特製貓掌造型奶泡，療癒感十足。", nutrition: { kcal: 155, carb: 13, fat: 8.5, protein: 6.2 } },
  coffee_4: { name: "雲朵焦糖瑪奇朵", price: 165, desc: "如雲朵般輕盈的奶泡與焦糖交織。", nutrition: { kcal: 180, carb: 22, fat: 9, protein: 5 } },
...
export default function App() {
  ...
  const handleReservationFromMenu = (itemName: string) => {
    setFormData(prev => {
      ...
      if (!currentMsg.startsWith(prefix)) return { ...prev, message: `${prefix}${itemName} x 1` };
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] text-[#4E342E] font-sans selection:bg-[#E67E22] selection:text-white">
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#FDFBF7]/80 backdrop-blur-md border-b border-[#5D4037]/10">
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 font-medium">
            {['about', 'cats', 'menu', 'contact'].map(id => (
              <button key={id} onClick={() => scrollToSection(id)} className="hover:text-[#E67E22]">{id === 'about' ? '關於我們' : id === 'cats' ? '店內貓咪' : id === 'menu' ? '精選菜單' : '預約訂位'}</button>
            ))}
          </div>
        ...
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="md:hidden bg-[#FDFBF7] border-b overflow-hidden">
              <div className="px-6 py-8 flex flex-col gap-6">
                {['about', 'cats', 'menu', 'contact'].map(id => (
                  <button key={id} onClick={() => scrollToSection(id)} className="text-left font-bold">{id === 'about' ? '關於我們' : id === 'cats' ? '店內貓咪' : id === 'menu' ? '精選菜單' : '預約訂位'}</button>
                ))}
              </div>
            </motion.div>
          )}
      ...
      <section id="hero" className="pt-40 pb-20 px-6 max-w-7xl mx-auto grid md:grid-cols-2 gap-12 items-center">
        <FadeInUp>
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#E67E22]/10 text-[#E67E22] rounded-full text-sm font-bold mb-6"><Sparkles size={16} /> 台北最療癒的角落</div>
          <h1 className="text-5xl md:text-7xl font-bold mb-6">在貓咪的呼嚕聲中<br />品味<span className="text-[#E67E22]">醇厚時光</span></h1>
          <Button onClick={() => scrollToSection('menu')}>查看菜單</Button>
        </FadeInUp>
      ...
      <section id="menu" className="py-24 bg-white px-6 max-w-7xl mx-auto">
        <div className="flex justify-center gap-4 mb-12">
          {['coffee', 'dessert', 'treat'].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab as any)} className={`px-6 py-3 rounded-full font-bold ${activeTab === tab ? 'bg-[#E67E22] text-white shadow-lg' : 'bg-[#FDFBF7]'}`}>{tab === 'coffee' ? '手沖咖啡' : tab === 'dessert' ? '精緻甜點' : '貓咪零食'}</button>
          ))}
        </div>
        <div className="grid md:grid-cols-3 gap-8">
          {CAFE_ASSETS.menu[activeTab].map((img, i) => {
            ...
            return (
              <div key={i} onClick={() => setSelectedItem({ ...detail, image: img })} className="bg-[#FDFBF7] rounded-3xl overflow-hidden cursor-pointer p-6 hover:shadow-xl transition-shadow">
      ...
      <section id="contact" className="py-24 bg-[#5D4037] text-white px-6">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-16">
          <div><h2 className="text-4xl font-bold mb-8">準備好與貓咪來場約會了嗎？</h2><div className="space-y-4"><p><MapPin className="inline mr-2" /> 台北市大安區療癒路 102 號</p><p><Clock className="inline mr-2" /> 11:00 - 20:00 (週一公休)</p></div></div>
          <div className="bg-white rounded-3xl p-8 text-[#4E342E]">
            {!isSubmitted ? (
              <form className="space-y-4" onSubmit={handleReservation}>
                ...
                <textarea ref={messageInputRef} className="w-full p-4 border rounded-xl h-32" placeholder="備註餐點" value={formData.message} onChange={e => setFormData({...formData, message: e.target.value})}></textarea>
                ...
                <Button className="w-full">確認送出</Button>
              </form>
            ) : <div className="text-center py-12"><CheckCircle2 className="mx-auto text-[#E67E22] mb-4" size={48} /><h3 className="text-2xl font-bold mb-2">預約成功！</h3><p>專人將儘速聯繫您。</p></div>}
          </div>
        </div>
      </section>
        ...
        {selectedCat && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-3xl overflow-hidden max-w-lg w-full">
              ...
              <div className="p-8"><h2 className="text-3xl font-bold mb-4">{selectedCat.name}</h2><p className="mb-6">{selectedCat.personality}</p><Button className="w-full" onClick={() => handleReservationFromCat(selectedCat.name)}>與牠約會</Button></div>
            </div>
          </div>
        )}
      </AnimatePresence>
        ...
        {selectedItem && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-3xl overflow-hidden max-w-lg w-full">
              ...
              <div className="p-8"><h2 className="text-2xl font-bold mb-4">{selectedItem.name}</h2><p className="mb-6">{selectedItem.desc}</p><Button className="w-full" onClick={() => handleReservationFromMenu(selectedItem.name)}>預約美味</Button></div>
            </div>
          </div>
        )}
      </AnimatePresence>
